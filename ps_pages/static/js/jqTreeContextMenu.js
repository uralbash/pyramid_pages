  ...
  $.fn.jqTreeContextMenu = function (menuElement, callbacks) {
    ...
    this.disable = function () {
      if (arguments.length === 0) {
        ...
        var items1 = arguments[0];
        if (typeof items1 !== 'object') {
          return;
        }
        $menuEl.find('li > a').each(function () {
          var hrefValue = $(this).attr('href');
          var value = hrefValue.slice(hrefValue.indexOf("#") + 1, hrefValue.length);
          if ($.inArray(value, items1) > -1) {
            $(this).closest('li').addClass('disabled');
            $(this).unbind('click');
          }
        });
        ...
        var items2 = arguments[1];
        nodeToDisabledMenuItems[nodeName] = items2;
      }
    };
      ...
      if (arguments.length === 0) {
        ...
        if (items2.length === 0) {
          ...
          for (var i = 0; i < items2.length; i++) {
            var idx = disabledItems.indexOf(items2[i]);
            if (idx > -1) {
